...
def _add_item(buckets: dict[str, list[dict[str, Any]]], item: dict[str, Any]) -> None:
    ...
    existing_level = _find_existing_level(buckets, key)
    ...
    if _item_score(item) < _item_score(existing_item):
        buckets[existing_level].pop(existing_index)
        buckets[missing_level].append(item)


def _find_existing_level(buckets: dict[str, list[dict[str, Any]]], key: str) -> str | None:
    for level in ("critical", "recommended", "optional"):
        if any(str(item.get("key") or "") == key for item in buckets[level]):
            return level
    return None
